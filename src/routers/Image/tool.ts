...
const get_img_ids_handler:{[key in ImageType]:(id:string)=>Promise<string[]>} ={
...
const update_img_ids_handler:{[key in ImageType]:(id_images:string[],id:string)=>Promise<any>} ={
...
const getIdsImageAndCheckOwner = async (user_input:z.infer<typeof post_img_schema>)=>{
...
const updateIdsArrDB = async (type:ImageType,img_ids:string[],id:string)=>{
 ...
 */
const uploadAndRemoveImage = async (user_input_image_ids:string[],files:fileUpload.UploadedFile[],delete_img_ids:string[])=>{
    //upload img to cloud
    const new_imgs_ids = await Promise.all(files.map((file)=>uploadImgToStorage(file)));
    //delete img from cloud
    await Promise.all(delete_img_ids.map((img_id)=>removeImgFromStorage(img_id)));
...
async function getImgsFromProject(id:string){
    const project = await prisma.project.findFirst({where:{id},select:{img_ids:true,portfolioData:{select:{user_id:true}}}});
    return {img_ids:project?.img_ids||[],owner_id:project?.portfolioData.user_id};
}
...
async function updateImgsFromProject(id_images:string[],id:string){
    const project = await prisma.project.update({where:{id},data:{img_ids:id_images},select:{img_ids:true}});
...
async function getImgsFromAchievement(id:string){
    const achievement = await prisma.achievement.findFirst({where:{id},select:{img_ids:true}});
...
async function updateImgsFromAchievement(id_images:string[],id:string){
    const achievement = await prisma.achievement.update({where:{id},data:{img_ids:id_images},select:{img_ids:true}});
    return achievement.img_ids;
}
...
async function getImgsFromExprience(id:string){
    const experience = await prisma.experience.findFirst({where:{id},select:{img_ids:true}});
...
async function updateImgsFromExprience(id_images:string[],id:string){
    const experience = await prisma.experience.update({where:{id},data:{img_ids:id_images},select:{img_ids:true}});
    return experience.img_ids;
}
